...
(function($) {
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'Imagem muito grande (max ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Tipo de arquivo inválido.');
        }
        else {
            callback(false, 'Tipo de arquivo não permitido.');
        }
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn span').text('Selecionar');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        //$fileTab.find('img').remove();
        $browseFileButton.find('span').text('Selecionar');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
					...
					$fileTab.find('img').attr("src",e.target.result);
					$fileTab.find('input:hidden').val(e.target.result);
                    $browseFileButton.find('span').text('Trocar');
